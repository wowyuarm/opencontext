...
def parse_brief_timestamp(workspace: str) -> Optional[str]:
    """Parse the generation timestamp from a brief's footer.

    Looks for: *Auto-generated by OpenContext | ... | Last updated: YYYY-MM-DD HH:MM*
    Returns ISO-ish string like "2026-02-17 14:22" or None.
    """
    content = read_brief(workspace)
    if not content:
        return None
    match = re.search(r"Last updated:\s*(\d{4}-\d{2}-\d{2}\s+\d{2}:\d{2})", content)
